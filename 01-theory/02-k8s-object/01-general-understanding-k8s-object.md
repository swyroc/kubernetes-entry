...
那么问题来了，你该如何使用K8S的对象呢？
    ...
    不管是你直接调用KS API，还是你通过kubectl命令行窗口来操作，最终发送给KS API的请求体中总会包含json格式的数据

    所以很自然的，我们的思维就过渡到如何去创建KS中的对象，来达到我们的诉求，注意这个地方是一个不小的思维转变
    例如，KS告诉我们它有一个Deloyment对象，这个对象可以用来表述XXXX，然后它抛出了创建规则，我们就按照某些规则去创建
    然后，创建的这些文件被KS识别，按照既定规则去完成创建动作。仅此而已。
```
